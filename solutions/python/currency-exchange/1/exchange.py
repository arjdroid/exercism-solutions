...
def get_leftover_of_bills(amount: float, denomination: int) -> float:
    ...
    return get_change(amount,get_value_of_bills(denomination, get_number_of_bills(amount, denomination)))
...
def exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    """
    new: float = exchange_money(budget, exchange_rate*((100+spread)/100))
    return get_value_of_bills(denomination, get_number_of_bills(new, denomination))
