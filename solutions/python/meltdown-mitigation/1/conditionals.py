...
def reactor_efficiency(voltage: float, current: float, theoretical_max_power: float) -> str:
    ...
    eff: float = (voltage * current) / theoretical_max_power * 100
...
def fail_safe(temperature: float, neutrons_produced_per_second: float, threshold: float) -> str:
    ...
    mvmt: float = temperature * neutrons_produced_per_second

    if mvmt < 0.90*threshold:
        return "LOW"
    elif mvmt >= 0.90*threshold and mvmt <= 1.10*threshold:
        return "NORMAL"
    else:
        return "DANGER"
